...
* **PPP: point to point protocol 点对点协议**
  1. 支持认证
  2. IPCP阶段会告知IP地址，不同网段间也可以互访
  3. 公有协议

**建立过程**

![image-20240630154218724](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630154218724.png)

### LCP

link control protocol：链路控制协议。

作用：二层链路建立、保活、拆除、认证（PAP、CHAP）、防环、压缩

#### 建立

**Configuration Request**

1. ID是设备用来标记请求包的，设备发送request后，只认与request中id相同的ack。
2. MRU（Maximum Receive Unit）：告诉对方自己的MTU，双方交换对方的MTU比较后，使用小MTU。

认证方：

![image-20240630155239926](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630155239926.png)

被认证方：

![image-20240630162009985](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630162009985.png)

**Configuration Ack**

如果对方Configuration Request中的所有Option都没问题，则回复**Configuration Ack**。

例如被认证方回复：

![image-20240630161301704](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630161301704.png)

**Configuration Nak**

1. 魔术字，用于防环，若收到Magic number A与自己的相同，回复Nak（Magic-number A）。
2. 被认证方配置的认证协议不符合认证发Request中Options中的认证协议时，回复Nak。

![image-20240630180144954](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630180144954.png)

**Configuration Reject**

1. 认证方发送的Request中有Authentication Protocol，而被认证方没有配置认证协议，回复Reject

![image-20240630172546635](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630172546635.png)

#### 拆除

**Termination Request**

![image-20240630162932635](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630162932635.png)
...
![image-20240630182254576](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630182254576.png)

**Authenticate-Request**

![image-20240630200659300](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630200659300.png)

**Authenticate-Ack**

![image-20240630200635150](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630200635150.png)

##### CHAP

1. 可以防止重放攻击，因为认证方每次发送的随机数都不相同，并且`Id + Value + password`计算hash值（MD5算法）该算法不可逆，攻击者无法推算出password。
2. 如果认证失败，继续认证2此，共认证`3次`，3次都失败，拆除链路。

![image-20240630181122594](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630181122594.png)

**Challenge**

认证方发送16Byte（128bit）的随机数Value。

![image-20240630181210656](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630181210656.png)

**Response**

被认证方用`Id + Value + password`计算hash值（MD5算法）。

![image-20240630181229595](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630181229595.png)

**Success**

认证方收到Response后，也计算`Id + Value + password`hash值（MD5算法），对比Response中的hash值、以及name。

如果相同，则认证成功，回复Success。

![image-20240630181244856](C:\Users\LNFeng\AppData\Roaming\Typora\typora-user-images\image-20240630181244856.png)

#### 防环

如果设备连续三次收到`Magic number`不合法的Nak，表示出现环路，LCP将不会建立链路。
